<template>
  <div>
    <h1 class="box-title">Deferred</h1>
    <ul>
      <draggable class="list-group"  group="tasks">
        <draggable class="list-group" :list="deferred" group="tasks">
      <li class="task" v-for="(task,row) in deferred" v-bind:key="row" v-bind:style="{backgroundColor:task.color}"
...
export default {
  ...
  watch: {
    deferred: {
      handler() {
        console.log('Tasks deferred changed')
        localStorage.setItem('deferred', JSON.stringify(this.deferred));
      },
      deep: true,
    },
  },
  mounted() {
    if (localStorage.getItem('deferred')) this.deferred = JSON.parse(localStorage.getItem('deferred'));
  },
  data() {
    return {
      isHidden: true,
  ...
  methods: {
    ...
    SaveTask() {
      ...
      this.deferred.push(inputObject)
    ...
    ModalOpen(task) {
      this.$modal.show('deferred-modal')
      this.activeTask = task
    },
    submitDescription() {
      this.$modal.hide('deferred-modal')
      console.log(this.activeTask)
    },
     DeleteTask(task) {
      this.deferred.splice(this.deferred.indexOf(task), 1);
    }
