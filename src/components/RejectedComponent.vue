<template>
  <div>
    <h1 class="box-title">Rejected</h1>
    <ul>
        <draggable class="list-group"  group="tasks">
        <draggable class="list-group" :list="rejected" group="tasks">
      <li class="task" v-for="(task,row) in rejected" v-bind:key="row" v-bind:style="{backgroundColor:task.color}"
...
export default {
  ...
  watch: {
    rejected: {
      handler() {
        console.log('Tasks rejected changed')
  ...
  mounted() {
    if (localStorage.getItem('rejected')) this.rejected = JSON.parse(localStorage.getItem('rejected'));
  },
  data() {
    return {
      isHidden: true,
  ...
  methods: {
    ...
    SaveTask() {
      ...
      this.rejected.push(inputObject)
    ...
    ModalOpen(task) {
      this.$modal.show('rejected-modal')
      this.activeTask = task
    },
    submitDescription() {
      this.$modal.hide('rejected-modal')
      console.log(this.activeTask)
    },
     DeleteTask(task) {
      this.rejected.splice(this.rejected.indexOf(task), 1);
    }
